...
JWT-based authentication:: When authentication is enabled, the proxy validates every incoming JSON web token (JWT) request against the Hub's OIDC provider by using its JWKS endpoint:
+
[subs="+quotes"]
----
