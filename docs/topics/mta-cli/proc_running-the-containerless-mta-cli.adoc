...
[role="_abstract"]
To analyze applications in environments where a container engine is not available or required, you can run the {ProductFullName} command-line interface (CLI) in containerless mode. This way, you can run assessments directly on your local machine by using the provided shell script, bypassing the need for Podman or Docker.

[IMPORTANT]
====
In {ProductShortName} 7.2.0 and later, the containerless CLI is the default mode. To enable container runtime usage for the analysis of Java applications, you must set the `--run-local` flag to `false`:
